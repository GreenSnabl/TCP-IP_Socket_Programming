...
int main(int argc, char *argv[]){
	...
	// file descriptor listen-socket und file descriptor connection-socket
	// server-adresse
	// client adresse
	// process thread
	...
	/* Socket settings */
	listenfd = socket(AF_INET, SOCK_STREAM, AF_UNSPEC);
	...
	*
	* the declarator is signal. Within the parenthesis, suffixes are higher precedence than prefixes, 
	* so signal is a function taking two args 
	* the (int signo, void *(func)(int)) part,
	* that returns a pointer (the prefix *) to a function taking 
	* a single int arg (the (int) on the end), and returning void
	*
	* Also:
	*
	* The type signature of the signal function is a bit more clear when a typedef is used 
	* for the function pointers that are passed around:
	*
	* typedef void (*sighandler_t)(int);
	* sighandler_t signal(int signo, sighandler_t func);
	*
	*/
	signal(SIGPIPE, SIG_IGN);   // SIGPIPE -> int, SIG_IGN -> funktionsweise/funktion (ignoriere signale)
